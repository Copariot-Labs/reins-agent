...
pub async fn generate(text: &str, voice: &str) -> Result<Vec<u8>> {
    ...
    Err(MeuxeError::Tts("Reins TTS: all endpoints failed".into()))
}
...
async fn generate_audio(text: &str, voice: &str, endpoint: &str) -> Result<Vec<u8>> {
    ...
    let resp = client()
        ...
        .send()
        .await
        .map_err(|e| MeuxeError::Tts(format!("Reins TTS request failed: {e}")))?;
    ...
    let bytes = resp
        .bytes()
        .await
        .map_err(|e| MeuxeError::Tts(format!("Reins TTS read error: {e}")))?;
