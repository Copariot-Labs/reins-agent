...
export function useChat() {
  ...
  const upsertToolCall = useCallback((call: ToolCallStatus) => {
    setTimeline((prev) => {
      const index = prev.findIndex(
        (item) => item.kind === 'tool' && item.id === call.requestId,
      );
      if (index === -1) {
  ...
  const send = useCallback(
    async (characterId: string, message: string, requestId: string) => {
      ...
      const unlistenText = await listen<TextChunkPayload>(
        'chat:text-chunk',
        (event) => {
      ...
      const unlistenError = await listen<ChatErrorPayload>(
        ...
        (event) => {
          ...
          const errorMessage =
            ...
            'An unknown Reins connection error occurred.';
          ...
          setTimeline((previousTimeline) => [
            ...previousTimeline,
            {
              ...
              kind: 'assistant',
              text: `Reins connection error: ${errorMessage}`,
            },
          ]);
  ...
  const setOnAudioFailed = useCallback(
    (cb: (data: AudioFailedPayload) => void) => {
