...
export function getApiKey(): string {
  return localStorage.getItem('reins_api_key') || '';
}
...
export function setApiKey(key: string) {
  localStorage.setItem('reins_api_key', key);
}

export function clearApiKey() {
  localStorage.removeItem('reins_api_key');
}
...
function bodyHasProfileSelector(body: BodyInit | null | undefined): boolean {
  ...
  try {
    ...
    return (
      typeof parsed?.profile === 'string' && parsed.profile.trim().length > 0
    );
  } catch {
    return false;
  }
...
function shouldAttachProfileHeader(
  path: string,
  options: RequestInit,
): boolean {
  try {
    ...
    if (isProfileWideSessionCollection(path.split('?')[0] || path))
      return false;
  }
...
function emitAuthNotice(kind: 'expired' | 'forbidden') {
  ...
  window.dispatchEvent(
    new CustomEvent('hermes-auth-notice', { detail: { kind } }),
  );
}
