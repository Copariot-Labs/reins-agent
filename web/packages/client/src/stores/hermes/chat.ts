import {
  ...
  type ContentBlock as ContentBlockImport,
...
function isToolOutputError(output: unknown): boolean {
  ...
  try {
    ...
    if (parsed && typeof parsed === 'object') {
      ...
      if (record.error != null && String(record.error).trim() !== '')
        return true;
    }
  } catch {
    return false;
...
async function uploadFiles(
  ...
  const token = localStorage.getItem('reins_api_key') || '';
...
function legacyStorageKey(): string | null {
  return getProfileName() === 'default' ? LEGACY_STORAGE_KEY : null;
}
...
function recoverStorageQuota() {
  try {
    ...
    if (keysToRemove.length > 0) {
      console.log(
        `Recovered storage: cleared ${keysToRemove.length} old session cache entries`,
      );
    }
  } catch {
    // ignore
...
export const useChatStore = defineStore('chat', () => {
  ...
  function isSessionLive(sessionId: string): boolean {
    return (
      streamStates.value.has(sessionId) || serverWorking.value.has(sessionId)
    );
  }
  ...
  async function loadSessions(
    ...
    try {
      ...
      const runtimeByIdBefore = new Map(
  ...
  async function refreshActiveSession(): Promise<boolean> {
    ...
    try {
      ...
      const detail = await fetchSessionMessagesPage(
        sid,
        0,
        limit,
  ...
  function createSession(
    options: { profile?: string; model?: string; provider?: string } = {},
  ): Session {
    const session: Session = {
      id: uid(),
      profile:
        options.profile || useProfilesStore().activeProfileName || 'default',
      title: '',
      source: 'cli',
      messages: [],
      createdAt: Date.now(),
  ...
  async function switchSession(sessionId: string, focusId?: string | null) {
    ...
    activeSession.value =
      sessions.value.find((s) => s.id === sessionId) || null;
          ...
          (data) => {
            ...
            if (Array.isArray((data as any).queueMessages)) {
              replaceQueuedUserMessages(
                sessionId,
                normalizeQueuedUserMessages((data as any).queueMessages),
              );
            ...
            if ((data as any).contextTokens != null)
              target.contextTokens = (data as any).contextTokens;
            if (data.messages?.length) {
              target.messages = mapHermesMessages(data.messages as any[]);
              target.loadedMessageCount =
                data.messageLoadedCount ?? data.messages.length;
              target.messageTotal =
                data.messageTotal ??
                target.messageCount ??
                target.loadedMessageCount;
              target.messageCount = target.messageTotal;
              target.hasMoreBefore =
                data.hasMoreBefore ??
                target.loadedMessageCount < target.messageTotal;
            }
            if (!target.title) {
  ...
  async function loadOlderMessages(
    ...
    const target = sessions.value.find((s) => s.id === sessionId);
    if (!target || target.isLoadingOlderMessages || !target.hasMoreBefore)
      return false;
    ...
    try {
      ...
      const olderMessages = mapHermesMessages(page.messages).filter(
  ...
  function handleSubagentEvent(sessionId: string, evt: RunEvent) {
    ...
    const subagentId = String(
      (evt as any).subagent_id || `${(evt as any).task_index ?? 0}`,
    );
    ...
    const duration = Number(
      (evt as any).duration_seconds ?? (evt as any).duration,
    );
  ...
  function handleSessionCommandEvent(evt: RunEvent) {
    ...
    if (
      action === 'title' &&
      target &&
      typeof (evt as any).title === 'string'
    ) {
  ...
  function updateQueuedUserMessage(
    sessionId: string,
    messageId: string,
  ...
  function dropQueuedUserMessage(
    sessionId: string,
    messageId: string,
  ): boolean {
    const queue = queuedUserMessages.value.get(sessionId);
  ...
  function normalizeQueuedUserMessages(rawMessages: unknown): Message[] {
    ...
    return rawMessages.flatMap((raw) => {
      ...
      const timestamp =
        typeof peer?.timestamp === 'number' && Number.isFinite(peer.timestamp)
  ...
  function replaceQueuedUserMessages(sessionId: string, messages: Message[]) {
    const existingById = new Map(
      (queuedUserMessages.value.get(sessionId) || []).map((message) => [
        message.id,
        message,
      ]),
    );
    const merged = messages.map((message) => ({
      ...
      ...message,
      attachments:
        existingById.get(message.id)?.attachments || message.attachments,
      queued: true,
    }));
    const nextMap = new Map(queuedUserMessages.value);
    if (merged.length > 0) {
  ...
  function consumeDequeuedQueueId(
    sessionId: string,
    messageId: string,
  ): boolean {
  ...
  function handleRunQueuedEvent(sessionId: string, evt: RunEvent) {
    ...
    const dequeuedId =
      (evt as any).dequeued_queue_id != null
        ? String((evt as any).dequeued_queue_id)
        : '';
    if (dequeuedId) {
      const existingQueue = queuedUserMessages.value.get(sessionId) || [];
      const dequeued = existingQueue.find(
        (message) => message.id === dequeuedId,
      );
      if (Array.isArray((evt as any).queued_messages)) {
        const queued = normalizeQueuedUserMessages(
          (evt as any).queued_messages,
        );
      ...
      if (
        dequeued &&
        !getSessionMsgs(sessionId).some((message) => message.id === dequeued.id)
      ) {
    ...
    if (
      (queuedUserMessages.value.get(sessionId) || []).some(
  ...
  function setPendingApproval(evt: RunEvent) {
    ...
    const rawChoices = Array.isArray((evt as any).choices)
      ? (evt as any).choices
      : ['once', 'session', 'deny'];
    const choices = rawChoices.filter(
      (choice: unknown): choice is PendingApproval['choices'][number] =>
        choice === 'once' ||
        choice === 'session' ||
        choice === 'always' ||
        choice === 'deny',
  ...
  async function sendMessage(
    ...
    const shouldSendInitialSessionConfig = activeSession.value
      ? activeSession.value.messageCount == null ||
        activeSession.value.messageCount === 0
      : false;
    const isBridgeSlashCommand = content.trim().startsWith('/');
    const isBridgeCompressCommand =
      isBridgeSlashCommand && /^\/compress(?:\s|$)/i.test(content.trim());
    const isBridgePlanCommand =
      isBridgeSlashCommand && /^\/plan(?:\s|$)/i.test(content.trim());
    const isBridgeGoalCommand =
      isBridgeSlashCommand && /^\/goal(?:\s|$)/i.test(content.trim());
    const wasLiveBeforeSend = isSessionLive(sid);
    const shouldQueue =
      wasLiveBeforeSend && (!isBridgeSlashCommand || isBridgePlanCommand);
    ...
    try {
      ...
      if (attachments && attachments.length > 0) {
        ...
        if (shouldQueue && userMsg.attachments) {
          ...
          updateQueuedUserMessage(sid, userMsg.id, {
      ...
      const runPayload = {
        ...
        profile:
          ...
          undefined,
        model: shouldSendInitialSessionConfig
          ? sessionModel || undefined
          : undefined,
        provider: shouldSendInitialSessionConfig
      ...
      const applyReconnectResume = (data: ResumeSessionPayload) => {
        ...
        if (Array.isArray(data.queueMessages)) {
          replaceQueuedUserMessages(
            sid,
            normalizeQueuedUserMessages(data.queueMessages),
          );
        } else if (!data.queueLength) {
        ...
        if (Array.isArray(data.messages)) {
          ...
          target.hasMoreBefore =
            data.hasMoreBefore ??
            target.loadedMessageCount < target.messageTotal;
          const lastAssistant = [...target.messages]
            .reverse()
          ...
          for (const evt of data.events) {
            ...
            switch (e.event) {
              ...
              case 'compression.completed': {
                const afterTokens =
                  (e as any).contextTokens || (e as any).afterTokens || 0;
                setCompressionState(sid, {
                  compressing: false,
                ...
                if ((e as any).contextTokens != null)
                  target.contextTokens = (e as any).contextTokens;
                break;
              }
              case 'abort.started':
                setAbortState({ aborting: true, synced: null });
      ...
      const ctrl = startRunViaSocket(
        ...
        (evt: RunEvent) => {
          switch (evt.event) {
            ...
            case 'compression.completed': {
              const afterTokens =
                (evt as any).contextTokens || (evt as any).afterTokens || 0;
              setCompressionState(sid, {
                compressing: false,
            ...
            case 'abort.completed': {
              setAbortState({
                aborting: false,
                synced: (evt as any).synced ?? false,
              });
            ...
            case 'tool.started': {
              ...
              const msgs = getSessionMsgs(sid);
              const toolCallId = (evt as any).tool_call_id as
                | string
                | undefined;
              const last = activeAssistantMessageId
                ? msgs.find((m) => m.id === activeAssistantMessageId)
              ...
              if (existingTool) {
                updateMessage(sid, existingTool.id, {
                  ...
                  toolArgs:
                    typeof (evt as any).arguments === 'string'
  ...
  function handlePeerUserMessage(evt: RunEvent) {
    ...
    if (
      messageId &&
      (queuedUserMessages.value.get(sid) || []).some(
        (msg) => msg.id === messageId,
      )
    ) {
      serverWorking.value.add(sid);
    ...
    const wasDequeued = messageId
      ? consumeDequeuedQueueId(sid, messageId)
      : false;
  ...
  function handleGlobalSessionCommand(evt: RunEvent) {
    const sid = evt.session_id;
    if (!sid || activeSessionId.value !== sid || !activeSession.value) return;
    const shouldAttachToStartedRun =
      (evt as any).started === true && (evt as any).terminal === false;
    handleSessionCommandEvent(evt);
          ...
          resumeSession(
            ...
            (data) => {
              ...
              if (data.messages?.length && activeSession.value) {
                activeSession.value.messages = mapHermesMessages(
                  data.messages as any[],
                );
                activeSession.value.loadedMessageCount =
                  data.messageLoadedCount ?? data.messages.length;
                activeSession.value.messageTotal =
                  data.messageTotal ??
                  activeSession.value.messageCount ??
                  activeSession.value.loadedMessageCount;
                activeSession.value.messageCount =
                  activeSession.value.messageTotal;
                activeSession.value.hasMoreBefore =
                  data.hasMoreBefore ??
                  activeSession.value.loadedMessageCount <
                    activeSession.value.messageTotal;
              }
  ...
  const thinkingObservation = new Map<
    string,
  ...
  function noteThinkingDelta(
    messageId: string,
    prevContent: string,
    nextContent: string,
  ) {
    const { startedAtBoundary, endedAtBoundary } = detectThinkingBoundary(
      prevContent,
      nextContent,
    );
