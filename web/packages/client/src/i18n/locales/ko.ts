export default {
  ...
  sidebar: {
    ...
    connected: '연결됨',
    disconnected: '연결 끊김',
    updateTip: '최신 Reins 버전을 다운로드하고 설치',
    updateVersion: 'v{version}(으)로 업그레이드',
    reloadClientVersion: 'v{version}(으)로 새로고침',
    updating: '업데이트 중...',
    updateSuccess: '업데이트를 시작했습니다. 완료되면 Reins가 자동으로 다시 시작됩니다.',
  ...
  chat: {
    ...
    openHistory: '기록 열기',
    hermesHistory: 'Reins 기록',
    historyScopeHint: '현재 profile의 Reins 기록 세션을 소스별로 읽기 전용으로 봅니다.',
    noSessions: '세션 없음',
    newChat: '새 채팅',
    ...
    searchScope: '검색 범위: Web UI 로컬 세션 DB만 포함하며 읽기 전용 Reins 기록 세션은 포함하지 않습니다.',
  ...
  plugins: {
    title: '플러그인',
    refresh: '새로고침',
    notice: '탐색 가능한 Reins 플러그인 manifest의 읽기 전용 인벤토리입니다. 탐색 메타데이터는 플러그인 코드를 로드하지 않고 읽습니다. v1의 관리 작업은 CLI에 유지되며, 변경 사항은 새 Reins 세션에서 적용됩니다.',
  ...
  models: {
    ...
    copilotAddDetected: '이 컴퓨터에서 GitHub Copilot OAuth 토큰이 감지되었습니다. 추가를 클릭하여 Reins에서 Copilot을 활성화하세요.',
    copilotAddSourceEnv: '출처: ~/.reins/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: '출처: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: '출처: VS Code Copilot 확장 (apps.json)',
    copilotDeleteHintEnv: '이 작업은 ~/.reins/.env의 COPILOT_GITHUB_TOKEN을 지웁니다. 다른 도구에는 영향이 없습니다.',
    copilotDeleteHintGhCli: 'Copilot이 Reins에서 숨겨집니다. gh CLI 로그인에는 영향이 없으며 `gh auth status`는 여전히 로그인 상태를 표시합니다.',
    copilotDeleteHintAppsJson: 'Copilot이 Reins에서 숨겨집니다. VS Code Copilot 확장 로그인에는 영향이 없습니다.',
    customBadge: '커스텀',
    previewBadge: '프리뷰',
    disabledBadge: '사용 불가',
    ...
    visibilityHint: 'Web UI의 모델 선택기와 모델 페이지 표시에만 영향을 줍니다. Reins CLI의 provider/model 설정은 변경되지 않으며 실제 호출은 원본 모델 ID를 사용합니다.',
    ...
    aliasManage: '표시 이름',
    aliasHint: 'Web UI 표시 이름만 변경합니다. Reins에는 여전히 원본 모델 ID가 전달됩니다.',
    aliasEdit: '이름 변경',
  ...
  profiles: {
    ...
    rename: '이름 변경',
    delete: '삭제',
    switchTo: 'Reins Profile 전환',
    switchConfirm: '`reins profile use {name}`를 실행하고 Reins CLI active profile을 변경합니다. 계속하시겠습니까?',
    switchSuccess: 'Reins active profile이 "{name}"(으)로 전환되었습니다',
    switchFailed: 'Reins Profile 전환 실패. 게이트웨이를 수동으로 재시작해야 할 수 있습니다.',
  ...
  platform: {
    ...
    weixinTokenHint: 'weixin CLI QR 로그인에서 가져오기 (reins weixin)',
  ...
  skillsUsage: {
    title: '스킬 사용량',
    subtitle: 'Reins 세션의 스킬 로드와 편집을 추적합니다',
    refresh: '새로고침',
  ...
  changelog: {
    ...
    new_0_6_3_2: 'History 에 Reins CLI 세션을 Web UI 로컬 기록으로 가져오는 컨트롤을 추가하고 메시지 구조를 더 안전하게 정규화합니다',
    new_0_6_3_3: 'Provider 설정에서 기본 base URL 편집을 지원하고 LM Studio 를 내장 Provider 로 추가했으며 LM Studio /models 실시간 검색을 지원합니다',
    new_0_6_3_4: 'Web UI bridge 를 통해 전송되는 OpenRouter 요청에 Reins Web UI 앱 attribution headers 가 포함됩니다',
    ...
    new_0_6_2_10: '기본 컨텍스트 길이가 Reins 표준에 맞춰 256,000 tokens로 변경되었습니다',
    ...
    new_0_5_34_2: '사용자 지정 Reins 모델을 backend 관리 설정에 저장하고 모델 추가/삭제 endpoint 추가',
    ...
    new_0_5_34_8: 'Windows에서 Kanban 기능이 비정상이라면 최신 Kanban 데이터베이스 마이그레이션 수정이 포함된 Reins로 업그레이드하세요',
    ...
    new_0_6_0_1: '계정별 및 Profile별 관리가 세션, 모델, 사용량, Kanban, 작업, 업로드, 미디어, 관련 Reins API를 일관되게 보호합니다',
    new_0_6_0_2: '내장 미디어 Skills는 생성된 서버 토큰을 미디어 엔드포인트에만 사용하고 요청한 Profile에서 fun-codex/xAI 자격 증명을 확인합니다',
    new_0_6_0_3: '단일 채팅과 그룹 채팅이 현재 Reins Profile을 run instructions에 주입해 Skills가 X-Reins-Profile을 보낼 수 있습니다',
    ...
    new_0_6_1_2: 'Clarify 및 확인 응답이 인증된 채팅 socket을 통해 Reins bridge로 전달되며 응답 경로 테스트가 추가되었습니다',
