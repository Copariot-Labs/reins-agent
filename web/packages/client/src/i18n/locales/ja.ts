export default {
  ...
  sidebar: {
    ...
    connected: '接続済み',
    disconnected: '未接続',
    updateTip: '最新バージョンの Reins をダウンロードしてインストール',
    updateVersion: 'v{version} にアップグレード',
    reloadClientVersion: 'v{version} に再読み込み',
    updating: '更新中...',
    updateSuccess: '更新を開始しました。完了すると Reins は自動的に再起動します。',
  ...
  chat: {
    ...
    contextEditHint: '一般的な値：256k (Reins 既定), 128k (GPT-4), 32k (GPT-3.5)',
    contextEditSave: '保存',
    ...
    emptyState: 'Reins Agent と会話を開始しましょう',
    ...
    openHistory: '履歴を開く',
    hermesHistory: 'Reins 履歴',
    historyScopeHint: '現在の profile の Reins 履歴セッションをソース別に読み取り専用で表示します。',
    ...
    searchScope: '検索範囲: Web UI のローカルセッション DB のみ。読み取り専用の Reins 履歴セッションは含まれません。',
  ...
  plugins: {
    title: 'プラグイン',
    refresh: '更新',
    notice: '検出可能な Reins プラグイン manifest の読み取り専用インベントリです。検出メタデータはプラグインコードを読み込まずに取得します。v1 の管理操作は CLI のままで、変更は新しい Reins セッションで有効になります。',
  ...
  models: {
    ...
    copilotAddDetected: 'このマシンで GitHub Copilot OAuth トークンを検出しました。「追加」をクリックして Reins で Copilot を有効化します。',
    copilotAddSourceEnv: 'ソース: ~/.reins/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'ソース: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'ソース: VS Code Copilot 拡張機能 (apps.json)',
    copilotDeleteHintEnv: 'この操作で ~/.reins/.env の COPILOT_GITHUB_TOKEN を消去します。他のツールには影響しません。',
    copilotDeleteHintGhCli: 'Copilot は Reins 上で非表示になります。gh CLI のログインには影響しません — `gh auth status` は引き続きログイン状態を表示します。',
    copilotDeleteHintAppsJson: 'Copilot は Reins 上で非表示になります。VS Code Copilot 拡張機能のログインには影響しません。',
    customBadge: 'カスタム',
    previewBadge: 'プレビュー',
    disabledBadge: '利用不可',
    ...
    visibilityHint: 'Web UI のモデル選択とモデルページの表示だけに影響します。Reins CLI の provider/model 設定は変更されず、実際の呼び出しは元のモデル ID を使います。',
    ...
    aliasManage: '表示名',
    aliasHint: 'Web UI の表示名だけを変更します。Reins には元のモデル ID が送信されます。',
    aliasEdit: '名前変更',
  ...
  profiles: {
    ...
    rename: '名前変更',
    delete: '削除',
    switchTo: 'Reins Profile を切り替え',
    switchConfirm: '`reins profile use {name}` を実行し、Reins CLI の active profile を切り替えます。続行しますか？',
    switchSuccess: 'Reins active profile を「{name}」に切り替えました',
    switchFailed: 'Reins Profile の切り替えに失敗しました。ゲートウェイの手動再起動が必要な場合があります。',
  ...
  platform: {
    ...
    weixinTokenHint: 'weixin CLI の QR ログインから取得 (reins weixin)',
  ...
  skillsUsage: {
    title: 'スキル使用状況',
    subtitle: 'Reins セッションでのスキル読み込みと編集を追跡します',
    refresh: '更新',
  ...
  changelog: {
    ...
    new_0_6_3_2: 'History に Reins CLI セッションを Web UI のローカル履歴へインポートする操作を追加し、メッセージ構造をより安全に正規化します',
    new_0_6_3_3: 'Provider 設定で組み込み base URL を編集できるようになり、LM Studio を組み込み Provider として追加し、LM Studio /models のライブ取得に対応しました',
    new_0_6_3_4: 'Web UI bridge 経由の OpenRouter リクエストに Reins Web UI のアプリ attribution headers を付与するようになりました',
    ...
    new_0_6_2_10: '既定のコンテキスト長を Reins 標準に合わせて 256,000 tokens に変更しました',
    ...
    new_0_5_34_2: 'カスタム Reins モデルを backend 管理設定に永続化し、モデル追加/削除 endpoint を追加',
    ...
    new_0_5_34_8: 'Windows で Kanban 機能が異常な場合は、最新の Kanban データベース移行修正を取得するため Reins をアップグレードしてください',
    ...
    new_0_6_0_1: 'アカウント別・Profile 別の管理により、セッション、モデル、使用量、Kanban、ジョブ、アップロード、メディア、関連 Reins API を一貫して保護します',
    new_0_6_0_2: '内蔵メディア Skills は生成されたサーバートークンをメディア endpoint のみに使用し、要求された Profile から fun-codex/xAI 認証情報を解決します',
    new_0_6_0_3: '単一チャットとグループチャットは現在の Reins Profile を run instructions に注入し、Skills が X-Reins-Profile を送れるようにします',
    ...
    new_0_6_1_2: 'Clarify と確認の返信が、認証済みチャット socket 経由で Reins bridge まで届くようになり、返信経路のテストも追加しました',
