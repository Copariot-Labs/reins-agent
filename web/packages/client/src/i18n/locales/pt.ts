export default {
  ...
  sidebar: {
    ...
    updateTip: 'Baixar e instalar a versao mais recente do Reins',
    updateVersion: 'Atualizar para v{version}',
    reloadClientVersion: 'Recarregar para v{version}',
    updating: 'Atualizando...',
    updateSuccess: 'A atualizacao foi iniciada. O Reins reiniciara automaticamente.',
  ...
  chat: {
    ...
    contextEditHint: 'Valores comuns: 256k (Reins padrão), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Inicie uma conversa com o Reins Agent',
    ...
    hermesHistory: 'Histórico Reins',
    historyScopeHint: 'Sessões do histórico Reins do perfil atual, somente leitura, agrupadas por origem.',
    ...
    searchScope: 'Escopo da busca: apenas banco local de sessões da Web UI; sessões históricas Reins somente leitura não são incluídas.',
...
jobTriggered: 'Job acionado',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Atualizar',
    notice: 'Inventário somente leitura dos manifests de plugins Reins detectáveis. Os metadados de descoberta são lidos sem carregar código de plugin. No v1, ações de gerenciamento ficam na CLI; mudanças entram em vigor em novas sessões Reins.',
  ...
  models: {
    ...
    copilotAddDetected: 'Foi detectado um token OAuth do GitHub Copilot nesta máquina. Clique em Adicionar para ativar o Copilot no Reins.',
    copilotAddSourceEnv: 'Origem: ~/.reins/.env (COPILOT_GITHUB_TOKEN)',
    copilotAddSourceGhCli: 'Origem: gh CLI (gh auth token)',
    copilotAddSourceAppsJson: 'Origem: extensão Copilot do VS Code (apps.json)',
    copilotDeleteHintEnv: 'Isto irá limpar o COPILOT_GITHUB_TOKEN em ~/.reins/.env. Outras ferramentas não são afetadas.',
    copilotDeleteHintGhCli: 'O Copilot ficará oculto no Reins. Sua sessão no gh CLI não é afetada — `gh auth status` continuará indicando que está conectado.',
    copilotDeleteHintAppsJson: 'O Copilot ficará oculto no Reins. A extensão Copilot do VS Code continuará conectada.',
    ...
    visibilityHint: 'Afeta apenas o seletor de modelos e a página de modelos da Web UI. A configuração provider/model da Reins CLI não é alterada; as chamadas ainda usam o ID original do modelo.',
    ...
    aliasHint: 'Altera apenas o nome exibido na Web UI. O Reins ainda recebe o ID original do modelo.',
    aliasEdit: 'Renomear',
  ...
  profiles: {
    ...
    rename: 'Renomear',
    delete: 'Excluir',
    switchTo: 'Trocar Reins Profile',
    switchConfirm: 'Isto executara `reins profile use {name}` e alterara o active profile do Reins CLI. Continuar?',
    switchSuccess: 'Reins active profile alterado para "{name}"',
    switchFailed: 'Falha ao trocar Reins Profile. O gateway pode precisar de reinicio manual.',
  ...
  platform: {
    ...
    weixinTokenHint: 'Do login QR da CLI weixin (reins weixin)',
  ...
  skillsUsage: {
    ...
    subtitle: 'Acompanhe carregamentos e edições de habilidades nas sessões Reins',
    refresh: 'Atualizar',
  ...
  changelog: {
    ...
    new_0_6_3_2: 'History agora inclui controles para importar sessoes Reins CLI para o historico local da Web UI com normalizacao de mensagens mais segura',
    ...
    new_0_6_3_4: 'Requests OpenRouter enviados pelo bridge da Web UI agora incluem headers de atribuicao do app Reins Web UI',
    ...
    new_0_6_2_10: 'O comprimento de contexto padrão agora segue o padrão Reins de 256.000 tokens',
    ...
    new_0_5_34_2: 'Persiste modelos Reins personalizados em configuração gerenciada pelo backend e adiciona endpoints de adicionar/remover modelo',
    ...
    new_0_5_34_8: 'Se o Kanban falhar no Windows, atualize o Reins para receber as correções mais recentes de migração do banco Kanban',
    ...
    new_0_6_0_1: 'A gestão por conta e por Profile agora protege de forma consistente sessões, modelos, uso, Kanban, jobs, uploads, mídia e APIs Reins relacionadas',
    ...
    new_0_6_0_3: 'Chat individual e em grupo injetam o Reins Profile atual nas instruções do run para que Skills enviem X-Reins-Profile',
    ...
    new_0_6_1_2: 'Respostas Clarify e de confirmação agora passam pelo socket de chat autenticado até a Reins bridge, com testes da rota de resposta',
