export default {
  ...
  sidebar: {
    ...
    updateTip: 'Telecharger et installer la derniere version de Reins',
    updateVersion: 'Mettre a jour vers v{version}',
    reloadClientVersion: 'Recharger pour v{version}',
    updating: 'Mise a jour...',
    updateSuccess: 'La mise a jour a commence. Reins redemarrera automatiquement.',
  ...
  chat: {
    ...
    contextEditHint: 'Valeurs courantes : 256k (Reins par défaut), 128k (GPT-4), 32k (GPT-3.5)',
    ...
    emptyState: 'Demarrer une conversation avec Reins Agent',
    ...
    hermesHistory: 'Historique Reins',
    historyScopeHint: 'Sessions d’historique Reins du profil actuel en lecture seule, regroupées par source.',
    ...
    searchScope: 'Portée de recherche : base locale des sessions Web UI uniquement ; les sessions d’historique Reins en lecture seule ne sont pas incluses.',
...
jobTriggered: 'Job declenche',
  ...
  plugins: {
    title: 'Plugins',
    refresh: 'Actualiser',
    notice: 'Inventaire en lecture seule des manifests de plugins Reins détectables. Les métadonnées de découverte sont lues sans charger le code des plugins. En v1, la gestion reste dans le CLI; les changements prennent effet dans les nouvelles sessions Reins.',
  ...
  models: {
    ...
    copilotAddDetected: 'Un token OAuth GitHub Copilot a été détecté sur cette machine. Cliquez sur Ajouter pour activer Copilot dans Reins.',
    ...
    copilotDeleteHintEnv: 'Cela supprimera COPILOT_GITHUB_TOKEN dans ~/.reins/.env. Les autres outils ne sont pas affectés.',
    copilotDeleteHintGhCli: 'Copilot sera masqué dans Reins. Votre connexion gh CLI n\'est pas affectée — `gh auth status` indiquera toujours que vous êtes connecté.',
    copilotDeleteHintAppsJson: 'Copilot sera masqué dans Reins. Votre extension Copilot de VS Code reste connectée.',
    ...
    visibilityHint: 'Affecte uniquement le sélecteur de modèles et la page Modèles de la Web UI. La configuration provider/model de Reins CLI n’est pas modifiée ; les appels utilisent toujours l’ID de modèle d’origine.',
    ...
    aliasHint: 'Modifie uniquement le nom affiché dans la Web UI. Reins reçoit toujours l’ID de modèle d’origine.',
    aliasEdit: 'Renommer',
  ...
  profiles: {
    ...
    rename: 'Renommer',
    delete: 'Supprimer',
    switchTo: 'Changer Reins Profile',
    switchConfirm: 'Cette action execute `reins profile use {name}` et change le active profile Reins CLI. Continuer ?',
    switchSuccess: 'Reins active profile change vers "{name}"',
    switchFailed: 'Echec du changement Reins Profile. La passerelle peut necessiter un redemarrage manuel.',
  ...
  platform: {
    ...
    weixinTokenHint: 'Depuis la connexion QR de la CLI weixin (reins weixin)',
  ...
  skillsUsage: {
    ...
    subtitle: 'Suivre les chargements et modifications de compétences dans les sessions Reins',
    refresh: 'Actualiser',
  ...
  changelog: {
    ...
    new_0_6_3_2: 'History ajoute des controles pour importer les sessions Reins CLI dans l historique local Web UI avec une normalisation de messages plus sure',
    ...
    new_0_6_3_4: 'Les requetes OpenRouter envoyees par le bridge Web UI incluent maintenant les headers d attribution Reins Web UI',
    ...
    new_0_6_2_10: 'La longueur de contexte par défaut suit désormais le standard Reins de 256 000 tokens',
    ...
    new_0_5_34_2: 'Persiste les modèles Reins personnalisés dans une configuration backend et ajoute les endpoints ajout/suppression',
    ...
    new_0_5_34_8: 'Si Kanban échoue sous Windows, mettez Reins à niveau pour obtenir les derniers correctifs de migration de la base Kanban',
    ...
    new_0_6_0_1: 'La gestion par compte et par Profile protège désormais de façon cohérente les sessions, modèles, usage, Kanban, jobs, uploads, médias et APIs Reins associées',
    ...
    new_0_6_0_3: 'Le chat individuel et le chat de groupe injectent le Reins Profile courant dans les instructions de run afin que les Skills envoient X-Reins-Profile',
    ...
    new_0_6_1_2: 'Les réponses Clarify et de confirmation passent maintenant par le socket de chat authentifié jusqu’au bridge Reins, avec des tests du chemin de réponse',
