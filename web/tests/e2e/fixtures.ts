...
export async function mockHermesApi(
  page: Page,
  options: MockHermesApiOptions = {},
) {
  ...
  await page.route('**/*', async (route: Route) => {
    ...
    if (pathname === '/api/auth/status') {
      await route.fulfill(
        jsonResponse({ hasPasswordLogin: true, username: 'playwright' }),
      );
      return;
    }
    ...
    if (pathname === '/api/hermes/auth/copilot/check-token') {
      await route.fulfill(
        jsonResponse({ has_token: false, source: null, enabled: false }),
      );
      return;
    }
    ...
    if (pathname === '/api/hermes/provider-models') {
      await route.fulfill(
        jsonResponse({ models: ['proxy-model-a', 'proxy-model-b'] }),
      );
      return;
    }
      ...
      await route.fulfill(
        jsonResponse({
          profiles: [
            ...
            {
              name: 'research',
              active: activeProfileName === 'research',
              model: 'test-model',
              gateway: 'test',
              alias: 'Research',
            },
          ...
          profiles: [
            {
              profile: 'default',
              bridge: {
                running: activeProfileName === 'default',
                profile: 'default',
                reachable: true,
              },
            ...
            },
            {
              profile: 'research',
              bridge: {
                running: activeProfileName === 'research',
                profile: 'research',
                reachable: true,
              },
    ...
    if (pathname === '/api/hermes/profiles/active') {
      ...
      await route.fulfill(
        jsonResponse({ success: true, active: activeProfileName }),
      );
      return;
    }
    ...
    await route.fulfill(
      jsonResponse(
        { error: `Unexpected mocked route: ${request.method()} ${pathname}` },
        404,
      ),
    );
  });
...
export async function authenticate(
  ...
  await page.addInitScript(
    (state: { storedToken: string; storedProfileName?: string }) => {
      const { storedToken, storedProfileName } = state;
      window.localStorage.setItem('reins_api_key', storedToken);
      if (
        storedProfileName &&
        !window.localStorage.getItem('hermes_active_profile_name')
      ) {
        window.localStorage.setItem(
          'hermes_active_profile_name',
          storedProfileName,
        );
      }
    },
        ...
        try {
          ...
          if (message.type === 'create') {
            this.__createSession(
              `term-${state.createdCount + 1}`,
              'bash',
              200 + state.createdCount,
            );
          }
          if (message.type === 'switch') {
            this.__emitMessage(
              JSON.stringify({ type: 'switched', id: message.sessionId }),
            );
          }
        } catch {}
      }
